...
class MatrixExtensionOneHundredPercentageTest {
    ...
    var expectedArray: FloatArray = darkArray

    @Before
    fun prepareTest() {
        matrix3f = Matrix3f(darkArray)
        matrix3f.serPercentageForMatrix(100)
    }
    ...
    fun checkValueAtIndex0() {
        var index = 0
        assert(
            matrix3f.array[index] == expectedArray[index],
            { "${matrix3f.array[index]} is not equal to ${expectedArray[index]}" })
    }
    ...
    @After
    fun printArrays() {
        println("MatrixExtensionOneHundredPercentageTest")
        for (i in 0..8) {
